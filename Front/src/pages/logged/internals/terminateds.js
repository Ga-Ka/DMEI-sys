...
export default function InternalsTerminateds() {
    ...
    useEffect(() => {
        ...
        axios.get(`http://10.10.135.100:3002/api/internals/terminateds/page=${(page-1)}/perPage=${perPage}`,)
        .then((res) => {
            setInternalsList(res.data);
        });

        axios.get('http://10.10.135.100:3002/api/internals/terminateds/')
        .then((res) => {
    ...
    const openInternal = (id) => {
        navigate(`/dmei-sys/internals/terminateds/${id}`);
    }

    //Add user
    const goToTerminate = () => {
        navigate(`/dmei-sys/internals/terminate`);
    }
    ...
    const tableData = internalsList?.map((internal) => {
        return {
            ...
            service: internal.service_performed
        };
    });
